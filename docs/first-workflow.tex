...
The agent can check the complete result with \texttt{zg config}. You can use
\texttt{zg config check} to check local credentials too, and
\texttt{zg config check -{}-live} when you want ZipperGen to contact the real
providers.
...
Starting is about to make real calls, so every model, assistant CLI, and connector is probed
...
now rather than in the middle of an approval. On Linux \texttt{zg deploy start}
installs a small \texttt{systemd} user service that restarts the workflow if it
fails. You never write that file, and \texttt{zg deploy status},
