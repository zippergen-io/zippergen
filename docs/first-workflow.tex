...
laptop. It wrote the workflow; it is not part of the running application, and
...
This particular workflow handles one approval and exits, which makes for a very
short-lived service. The version worth leaving running watches a mailbox and
never finishes on its own --- \texttt{examples/call\_intake.py} does exactly
that.
