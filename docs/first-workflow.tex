...
There is no privileged mode here. The agent and you have the same commands, and
the rest of this tutorial mixes both --- asking the agent when a question is
easier to say than to type, and running commands directly when they are short.
...
That is the boundary, and it is worth pausing on. The agent did everything it
could and then stopped --- not because a rule told it to be careful, but
because the token is typed into a prompt and never passed as an argument.
