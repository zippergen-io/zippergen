...
User will be asked through approvals.
\end{lstlisting}

The first command says \emph{which} chat. The second says \emph{who} is asked
there — the \texttt{User}, whose \texttt{@human} action is the approval. Without
the assignment the question still appears in whichever terminal is running the
workflow, which is right while you are developing and wrong for a deployment
nobody is watching.
...
> Everything validates. To send
  approvals to Telegram, run these
  yourself:
...
A server has no browser. Run \texttt{zg connector authorize google --scopes
...} on your own machine, then paste the \texttt{zg-google-v1...} result it
prints into \texttt{zg connector accept google} on the server. The credential
never travels through a shell argument.
\end{notebox}
