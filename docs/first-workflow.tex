...
before anything is sent. It does not stop on its own --- it is a service, and
the last section leaves it running on a server. It is short enough to read in
...
decision is always yes and the workflow never finishes --- which is the point.
...
\texttt{next\_unread\_message}, whose contract is one sentence --- \emph{return
the next message, waiting if there is none}. Here it takes the oldest
...
\texttt{User} tells it every round whether to continue --- and tells it once
more, at the end, not to. That is the \texttt{send\_decision} in the
\texttt{else}.
...
Stop it with \texttt{Ctrl-C}. When you want a run that ends by itself --- in a
test, or just to see a result --- give it a budget:
...
machine reboots, the service restarts --- and each time it picks up where it
was rather than re-reading a mailbox it has already answered.
...
because it has none. You wrote one protocol; ZipperGen worked out what each
participant runs, and worked it out in a way that cannot deadlock. As the
workflow grows — more participants, more loops, parallel regions — that is
