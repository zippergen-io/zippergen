...
\path{.gitignore} of its own so no project needs an entry for it. A clone has no
identity and is keyed by its own path until it is initialized as a new project.
Move the whole working directory and the existing identity moves with it. There
