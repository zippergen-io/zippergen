...
These are ordinary configuration, not something a workflow declares. A
provider environment variable such as \texttt{OLLAMA\_MAX\_TOKENS} still works
as an operational override where nothing is configured, but it is no longer
the way in: declaring a deployment field merely to set one would be a second
configuration mechanism for a value that belongs beside the model.
