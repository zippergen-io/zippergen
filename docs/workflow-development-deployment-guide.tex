...
\path{.zippergen/} & no & scratch \\
\texttt{ZIPPERGEN\_HOME} & no & credentials, runs, deployments \\
\bottomrule
\end{tabularx}
\end{center}

The split that matters is the last two rows. Everything above them travels with
the project; everything in them describes one machine. A colleague who clones
...
changes in one place. \texttt{configurations} says what a thing is;
...
The \texttt{Writer} has no branch, because it takes no part in the
...
real model would produce good text. Both matter; they are different questions.
\end{notebox}

\subsection{Durable runs}

A durable run records each step before taking it, in its own SQLite store. An
interrupted run resumes where it stopped, and a model call already made is not
made again.
...
own computer; accept on the server. The credential never appears in a command
...
\path{AGENTS.md} pointing at them; \texttt{zg skill} prints them.
...
command you can. What it \emph{cannot} do is obtain a credential it was never
given --- the Telegram token and Google authorization are typed, never passed
as arguments. If you want a harder boundary, give the agent a shell with
different credentials than your own.
...
is stuck the cause is usually an action that never returns --- a model call
...
\texttt{run} & execute; \texttt{-{}-durable}, \texttt{-{}-resume} \\
\texttt{status} / \texttt{trace} / \texttt{tasks} / \texttt{approve} & inspect and answer \\
\midrule
\multicolumn{2}{@{}l}{\textbf{Configure}} \\
\texttt{connector configure telegram|gmail|google-sheets} & save one \\
