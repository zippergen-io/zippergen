...
not need you to name the workflow.
...
\texttt{zg run -{}-resume -{}-run-id ID} & continue a particular one \\
\bottomrule
\end{tabularx}
\end{center}

A plain run leaves nothing behind, so \texttt{zg deploy status} afterwards finds
nothing. That is deliberate: throwaway runs should not accumulate. When you want
to come back to a run, to resume it, inspect it, or answer a human action
later, use \texttt{-{}-durable}.
...
command offers a hidden prompt. The key is saved in private storage on this
computer. It is never written to \path{zippergen.toml}. One provider key may
...
\texttt{zg model check} checks the selected model credentials without printing
them. Add \texttt{-{}-live} only when you want to make a small real provider
request. Use \texttt{zg model unassign TARGET} before removing a configuration
...
These are deliberately separate. Preparing writes the bundle, a managed Python
environment, and the service files, and reaches nothing. A deployment can be
prepared today and started next week. Starting is about to make real calls, so
every model, assistant CLI, and connector is probed first and a failure stops it.
...
\begin{tabularx}{0.95\linewidth}{@{}lX@{}}
\toprule
\texttt{zg deploy} & every deployment on this machine \\
\texttt{zg deploy status NAME} & is it running, and where is it \\
\texttt{zg deploy logs NAME} & what it has been doing \\
\texttt{zg inspect NAME} & where each participant's local program is now \\
\texttt{zg inspect NAME --watch} & keep that position open and refresh it in place \\
\texttt{zg trace NAME} & recent protocol events \\
\texttt{zg tasks NAME} & decisions waiting for a person \\
\texttt{zg approve NAME} & answer one \\
\texttt{zg deploy check NAME} & readiness checks, in detail \\
\texttt{zg deploy stop} / \texttt{restart NAME} & move the service \\
\texttt{zg deploy compact NAME} & reclaim space, the service must be stopped \\
\texttt{zg deploy remove NAME} & delete it \\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{enumerate}
  ...
  \item \texttt{zg deploy -{}-name production -{}-no-start}, then
        \texttt{zg deploy start production}.
\end{enumerate}
...
without a timeout, or a human action nobody has answered. \texttt{zg tasks}
...
\subsection{\texttt{zg deploy status} shows nothing after a run}
...
\texttt{status} / \texttt{trace} / \texttt{tasks} / \texttt{approve} & inspect and answer \\
\midrule
\multicolumn{2}{@{}l}{\textbf{Configure}} \\
\texttt{config} / \texttt{config check} & show or check all effective configuration \\
...
\texttt{deploy [-{}-name NAME] [-{}-no-start]} & prepare, and start \\
\texttt{start} / \texttt{stop} / \texttt{restart} & move the service \\
\texttt{logs} / \texttt{doctor} & operate \\
\texttt{compact} / \texttt{remove} & reclaim, delete \\
\bottomrule
\end{tabularx}
\end{center}
