...
There is no \texttt{restart}: it was exactly \texttt{stop} then
\texttt{start}, and its most tempting reading -- apply my changes -- was the
one thing it never did. Use \texttt{stop} then \texttt{deploy} to pick up an
edit; ZipperGen refuses \texttt{zg deploy} while the service runs so that this
choice is explicit rather than silent.

One pair is still worth reading twice:
