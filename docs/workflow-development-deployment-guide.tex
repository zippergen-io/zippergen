...
\texttt{zg deploy restart} restarts the already prepared bundle; it does not
apply source or project-configuration changes.
...
\texttt{zg deploy restart} & no & kept & stop and start in one step \\
\texttt{zg deploy reset} & no & \textbf{emptied} & archive the state, start the workflow over \\
\texttt{zg deploy remove} & --- & archived & unregister the service and take the deployment away \\
\bottomrule
\end{tabularx}
\end{center}

Read that table twice for the two commands people confuse:

\begin{itemize}
  \item \texttt{restart} is the only way to get running again without
        rebuilding. It will happily restart yesterday's bundle, so after
        editing the workflow or \path{zippergen.toml} you want \texttt{stop}
        then \texttt{deploy}, not \texttt{restart}. ZipperGen now refuses
        \texttt{zg deploy} while the service runs, precisely so that this
