...
\path{zippergen.toml} & yes & models, connectors, entry point, no secrets \\
...
provider is an attribute of the named configuration, not another object to
manage. In the examples, \texttt{approval-chat} is a name chosen by the user
and \texttt{telegram} is the provider. Bare \texttt{zg model} and
...
\texttt{zg model configure}, \texttt{zg model assign}, and
...
every model and connector is probed first and a failure stops it.
...
Starting probes every model and connector. The message names what failed. The
