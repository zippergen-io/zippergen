...
\date{Manual edition: 18 September 2026}
...
\path{docs/first-workflow.pdf} builds one small application end to end.
This manual explains the parts it passes over. If you have not
...
clone has no identity at all and is keyed by its own path. \texttt{zippergen
init} does not change that: it returns immediately when a manifest already
exists, and an identity is minted only when a project is created. A clone
therefore never inherits the original's private state, because it never learns
the original's identity. Move the whole working directory and both local values move with
...
\texttt{FAMILY} is \texttt{provider}, \texttt{model}, \texttt{assistant}, or
\texttt{connector}. Square brackets mark an optional value. The connector kind
is inferred when the selected connection supports only one.
...
To save the model choice in \path{zippergen.toml}, use guided configuration.
The credential command saves the key privately on this computer:
...
Provider connection [openai-main]: openai-main
Model name or path: gpt-4o-mini
Model configuration name [gpt-4o-mini]: writer
\end{lstlisting}

ZipperGen then offers to assign the configuration to a participant that uses a
model. Press Enter to skip assignment, or use \texttt{zg model assign} later.
Updating a configuration keeps its existing assignments.
...
If you already have a working model server, use its URL and an installed model.
You do not need a custom Ollama Modelfile. Guided provider setup offers the
usual endpoint:
...
Run \texttt{zg model configure} with no arguments to choose a model
interactively. For a local connection, it shows the server URL and asks the
server for its model list, then offers participant assignment. Discovery does
not generate text. If the server cannot be reached, you can retry after starting
it or enter a model name manually. Fully specified configuration commands remain
offline.

Use \texttt{zg model check NAME} to check generation after the connection works.
This does not establish that the model has enough context for your application.

\paragraph{A model on another computer.}
The address \texttt{127.0.0.1} refers to the computer running ZipperGen. If your
model runs on a GPU server, use its reachable API address or an SSH tunnel.
For an Ollama server listening on port 11434, run this on the computer running
ZipperGen, from any directory. Replace \texttt{YOUR\_GPU\_HOST} with your SSH
host or alias:
...
The tunnel runs in the background and makes the remote server available at
\path{http://127.0.0.1:11434/v1} on this computer. Reuse an existing tunnel
when one is already running. If you later deploy ZipperGen on another machine,
that machine needs its own working connection to the model server.
This command does not restore the tunnel after a restart. An unattended
deployment needs a connection that will be available when its service starts.
...
model after that long without a call. This uses Ollama's native API, so leave
it unset for other OpenAI-compatible servers.
...
A value outside a field's declared \texttt{choices} is refused, and a name the
workflow never declared is refused with the ones it did. Setting and forgetting
one field in a single command is refused rather than resolved silently, and
\texttt{-{}-json} reports what was written rather than falling back to prose. The same answers can
also be given while deploying, with \texttt{zg deploy -{}-set field=value},
which is convenient when you are deploying anyway.
...
\texttt{FAMILY rename OLD NEW} & rename one configuration and everything naming it \\
...
behaviours of the global workflow, up to generated control messages. Every
finite projected execution prefix can also be extended to a complete one. The
second property is the formal deadlock-freedom guarantee. The core results are
machine-checked in Lean 4. The parallel extension is established separately in
the EXPRESS/SOS paper.
