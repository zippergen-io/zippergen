...
Any model configuration accepts
\texttt{-{}-temperature VALUE} from 0 to 1. This is portable project behavior,
and an explicit \texttt{@llm(temperature=...)} on one action takes precedence.
Zero reduces sampling variation; it is not a determinism guarantee. Some
current model families removed sampling parameters, so configuration checks
reject an explicit unsupported value rather than silently dropping it.
...
configuration & which actual mailbox or chat & \path{zippergen.toml}, committed \\
...
\begin{tabularx}{0.95\linewidth}{@{}lX@{}}
\toprule
\texttt{zg deploy status} & is it running, what state does it hold, and are its runtime and workflow bundle current \\
...
That is a count of events, not a size. A workflow whose events carry large
values --- a whole email, a long model answer --- holds far more bytes at the
same budget than one passing short strings, so size it for what your workflow
actually produces.
...
\texttt{deploy compact/reset/remove} & trim history and logs, reset state, or delete \\
\bottomrule
\end{tabularx}
\end{center}
