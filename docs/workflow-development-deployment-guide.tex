...
\path{.zippergen/} & no & this checkout's generated identity; ignores itself \\
...
\path{.zippergen/} holds one generated value that keys this checkout's private
state. It must \emph{not} travel with a clone: two checkouts sharing one
identity would share one credential store. So it is not in
\path{zippergen.toml}, it is not committed, and the directory contains a
\path{.gitignore} of its own so no project needs an entry for it. Clone the
repository and the clone simply gets its own identity. Move the whole working
directory and the identity moves with it. There is no rule to remember.
...
Everything here is a choice somebody made. There is no schema stamp and no
generated identity, because neither is a choice.
...
all of them. \texttt{zg deploy status} prints the same table for a deployment
already running, together with the path of the profile derived from it.

A deployment configured before answers were kept in the project still holds
them only in its profile. The next \texttt{zg deploy} adopts them and writes
them into \path{zippergen.toml}; the table reports them as coming from
\texttt{this deployment} that once, and from \path{zippergen.toml} thereafter.
