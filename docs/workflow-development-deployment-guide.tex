...
A plain run leaves nothing behind, so \texttt{zg status} afterwards finds
...
\begin{tabularx}{0.95\linewidth}{@{}lX@{}}
\toprule
\texttt{zg status NAME} & is it running, and where is it \\
\texttt{zg logs NAME} & what it has been doing \\
...
\texttt{zg doctor NAME} & readiness checks, in detail \\
\texttt{zg stop} / \texttt{restart NAME} & move the service \\
\texttt{zg compact NAME} & reclaim space, the service must be stopped \\
\texttt{zg remove NAME} & delete it \\
\bottomrule
\end{tabularx}
\end{center}
...
\subsection{\texttt{zg status} shows nothing after a run}
