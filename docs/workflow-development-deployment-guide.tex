...
there is no run-reset operation. If this project's deployment is already
running, a foreground run warns before it can compete for Telegram updates,
mailbox files, or another external resource.
...
\texttt{run status/inspect/trace/tasks/approve} & observe or answer a durable run \\
\midrule
...
\texttt{deploy compact/reset/remove} & reclaim, reset, or delete \\
\bottomrule
\end{tabularx}
\end{center}
