...
Studio binds the callback to \texttt{127.0.0.1} on a temporary port. Suppose
the printed authorization URL contains callback port \texttt{48321}. Keep
Studio waiting in the first SSH terminal. In a second terminal on the local
computer, run:
\begin{lstlisting}[style=shell]
ssh -N -L 48321:127.0.0.1:48321 AI_SERVER
\end{lstlisting}
Copy the authorization URL printed by Studio into the local browser. After
Google redirects the browser to \texttt{localhost:48321}, the tunnel carries
the callback to Studio. Close the second tunnel after Studio reports success.
Use the actual temporary port from the URL, not \texttt{48321}.
\end{notebox}
