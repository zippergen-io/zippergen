...
browser computer. Managed deployments detect the same connector requirements
...
\begin{enumerate}
  \item Google authorization on the computer that has a browser
...
Run the displayed command on the computer that has the browser and downloaded
JSON file. The command asks for the file path, opens Google's consent page,
and prints the permissions that Google granted. It then prints one long
private result line. Return to the waiting Studio prompt and paste that line.
The Studio prompt is hidden because the result contains a refresh token.

The browser computer needs ZipperGen with Google support. After a published
installation, install it once with:

\begin{lstlisting}[style=shell]
pip install "zippergen[google]"
\end{lstlisting}

When testing the current source checkout before that release, run the same
displayed arguments through that checkout:

\begin{lstlisting}[style=shell]
uv run --project /path/to/zippergen --extra google \
  zippergen connector authorize google \
  --scopes gmail.modify,spreadsheets
\end{lstlisting}
...
through the hidden paste. The browser computer does not need network access to
