...
project and records it as the project's workflow in \path{zippergen.toml}.
The nested framework repository remains separate. A clone of this outer
project can resolve the same entry point without private Studio state. Named
model and connector configurations, their assignments, and connector bindings
are also written to this manifest as you configure them. A later clone already
knows those project decisions and asks only for its own endpoint and secrets.
...
Save and leave the editor. Implement the current specification, then inspect
...
Studio summarizes the changed files, assistant checks, and assistant report.
Deployment later uses the committed \path{zippergen.lock} implementation
record to detect a specification change. That record travels with the project,
so a fresh clone derives the same implementation state without private Studio
data. In an interactive Git project, Studio offers to commit the specification,
implementation files, lock, and any manifest entry it created as one unit. The
message is editable, only those related files are included, and Studio never
pushes. Declining the offer leaves every change in place for ordinary Git use.
...
The chosen provider, model, and default assignment are project configuration
in \path{zippergen.toml}. The local endpoint, idle-release policy, and API key
are site configuration. If this project is cloned onto another machine, enter
\texttt{project} there. Studio reports only the missing endpoint or key. It
does not ask you to recreate \texttt{call-extractor} or its assignment.
...
\begin{notebox}{The setup is driven by the project workflow}
...
mode, rate limit, and polling interval. Another project with another workflow
...
After troubleshooting, use
\texttt{deploy logs reset call-intake-check} to archive the visible history
and begin with an empty log view. This does not stop the deployment or alter
its durable workflow state.
...
participant once per second. Press Ctrl-C to close this view. The deployment
continues running. During normal polling, Mailbox may be waiting in
...
\texttt{deploy storage} shows how the polling trace and durable recovery data
are growing. It does not modify the deployment.
...
The \texttt{Project alignment} row in \texttt{deploy show} compares this
deployment with the current project. It covers the specification, accepted
workflow, models, idle policy, assistant, connectors, and local model
endpoint. Changing the project does not change the running service. Redeploy
to apply a reported difference. Enter \texttt{project} to see the workflow,
runs, and deployments together.
...
Trace retention already runs online and does not require this stop. The
optional compaction command removes only completed data covered by recovery
snapshots, rotates the service log, and keeps three log archives. It requires
the deployment to be stopped. Completed human tasks remain as audit records.
Studio also compacts the SQLite file and truncates its WAL safely.
The deployment can be started again with:
