...
def _ctrl_sends(owner: Lifeline, value: bool, receivers: list[Lifeline], tag: LitExpr) -> list[SendStmt]:
    """
    ...
    return [SendStmt(owner, (lit, tag), C) for C in receivers]
...
def _project(stmt: AnyStmt, A: Lifeline, counter: list[int]) -> LocalStmt:
    ...
    match stmt:
        ...
        case CoregionStmt(messages=messages):
            ...
            return cast(LocalStmt, seq(*(_project(msg, A, counter) for msg in messages)))
        ...
        case IfStmt(condition=c, owner=B, branch_true=p_true, branch_false=p_false):
            ...
            if A == B:
                ...
                return IfStmt(
                    ...
                    owner=B,
                    branch_true=seq(
                        *_ctrl_sends(B, True,  r_if, tag),
                        _project(p_true,  B, counter),
                    ),
                    branch_false=seq(
                        *_ctrl_sends(B, False, r_if, tag),
        ...
        case WhileStmt(condition=c, owner=B, body=p_body, exit_body=p_exit):
            ...
            if A == B:
                ...
                return WhileStmt(
                    ...
                    owner=B,
                    body=seq(
                        *_ctrl_sends(B, True,  r_while, tag),
                        _project(p_body, B, counter),
                    ),
                    exit_body=seq(
                        *_ctrl_sends(B, False, r_while, tag),
