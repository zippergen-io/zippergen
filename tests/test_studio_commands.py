...
def test_command_registry_has_unique_paths_and_derives_user_surfaces():
    ...
    assert ("deploy", "storage") in paths
    assert ("deploy", "storage", "compact") in paths
    ...
    assert "workflow review" not in full_help()
    assert "workflow accept" not in full_help()
...
def test_command_registry_owns_risk_and_natural_language_catalog():
    ...
    assert command_spec(["deploy", "storage"]).risk == "read-only"
