...
def test_tutorial_review_has_inspectable_deployment_contract():
    ...
    assert model["deployment"]["description"].startswith("Draft a reply")
    assert model["deployment"]["fields"]["max_retries"]["default"] == 2
    assert "openai_api_key" not in model["deployment"]["fields"]
    assert "anthropic_api_key" not in model["deployment"]["fields"]
